    ...
    for rule in rules:
        if rule == 0:
            ...
            if action_choice == "lock":
                return "permit"
            if action_choice == "unlock" and any(adult in occupants for adult in ["Dave", "Kate"]):
                return "permit"
        elif rule == 4:
            ...
            if time_minutes >= sunset_minutes or time_minutes <= sunrise_minutes:
                return "permit"
            elif action_choice == "turn off":
                return "permit"
        elif rule == 8:
            # can turn off if no child is home
            if not any(child in occupants for child in ["George", "Henry", "Evie"]) and action_choice == "turn off":
                return "permit"
        elif rule == 9:
            # can turn off if no parent is home
            if not any(parent in occupants for parent in ["Dave", "Kate"]) and action_choice == "turn off":
                return "permit"
        elif rule == 10:
            # can turn off if george isnt home
            if "George" not in occupants and action_choice == "turn off":
                return "permit"
        elif rule == 11:
            # can turn off if henry isnt home
            if "Henry" not in occupants and action_choice == "turn off":
                return "permit"
        elif rule == 12:
            # can turn off if evie isnt home
            if "Evie" not in occupants and action_choice == "turn off":
                return "permit"
        elif rule == 13:
            # can only adjust thermostat if adult is home
            if any(adult in occupants for adult in ["Dave", "Kate"]) and action_choice == "adjust":
                    return "permit"
        elif rule == 14:
            # can turn off if kate isnt home
            if "Kate" not in occupants and action_choice == "turn off":
                return "permit"
        elif rule == 15:
            # can turn off if dave isnt home
            if "Dave" not in occupants and action_choice == "turn off":
                return "permit"
    return "deny" # deny by default strategy
...
def ask_label(question, query):
    system_prompt = f"""you are to answer ONLY with a "permit" or "deny" based on these set of policies. they relate to a smart home with multiple devices.
                    ...
                    {query}""" # since the query can change, the query is parsed as a variable
...
def filter_query_for_user(query, user):
    # reduce policy text to lines that are relevant to the current user only
    if not user:
        return "No Policy found"

    # check if role based keywords are relevant
    is_parent = user in {"Dave", "Kate"}
    is_child = user in {"George", "Henry", "Evie"}
    user_lower = user.lower()

    filtered_lines = []
    for raw_line in query.splitlines(): # split the query into lines and check each one for relevance to the user
        line = raw_line.strip()
        if not line:
            continue
        ...
        # filter lines and keep only relevant ones
        if "everyone" in line_lower:
            keep = True
        elif user_lower in line_lower:
            keep = True
        elif "parents" in line_lower and is_parent:
            keep = True
        elif "dave and kate" in line_lower and is_parent:
            keep = True
        elif ("kids" in line_lower or "children" in line_lower) and is_child:
            keep = True

        if keep:
            filtered_lines.append(line) # add the policy line back

    return "\n".join(filtered_lines) if filtered_lines else "No Policy found" # combine all policies kept
...
# define each device category
device_types = {
    "front door lock": "security",
    "back door lock": "security",
    "bike shed lock": "security",
    "patio door lock": "security",
    "window sensors": "security",
    "alarm system": "security",
    "camera - driveway": "security",
    "camera - back garden": "security",
    "living room light": "lighting",
    "kitchen light": "lighting",
    "hallway light": "lighting",
    "garden light": "lighting",
    "playroom light": "lighting",
    "master bedroom light": "lighting",
    "George's bedroom light": "lighting",
    "Henry's bedroom light": "lighting",
    "Evie's bedroom light": "lighting",
    "upstairs thermostat": "heating",
    "downstairs thermostat": "heating",
    "Dave's electric bike charger": "appliances",
    "Kate's home studio speaker": "appliances",
    "wardrobe humidity monitor": "appliances",
    "Henry's tablet charger": "appliances",
    "Henry's night light": "appliances",
    "Evie's story time speaker": "appliances",
    "playroom projector": "appliances",
    "Dave's workshop smart plug": "smart plugs",
    "Dave's office PC plug": "smart plugs",
    "Kate's sewing machine plug": "smart plugs",
    "George's gaming PC plug": "smart plugs",
    "George's 3D printer plug": "smart plugs", 
    "Evie's karaoke plug": "smart plugs"
}
...
for i, q in enumerate(questions):
    ...
    # build the policy set depending on what device catergory was chosen
    if device_types.get(q[10]) == "security":
        query = """if fire alarm is going off, all members can unlock or lock any door or window
    ...
    elif device_types.get(q[10]) == "lighting":
        query = """Everyone can control the living room lights, kitchen lights, hallway lights
    ...
    elif device_types.get(q[10]) == "heating":                
        query = """Only Dave and Kate can adjust both upstairs and downstairs thermostats
                    The kids can change the upstairs thermostat if a parent is home"""
    elif device_types.get(q[10]) == "appliances":
        query = """Only Dave can control the electric bike charger
    ...
    elif device_types.get(q[10]) == "smart plugs":
        query = """Only Dave can access the workshop plug
    ...
    # further reduce the policy set to only user relevance               
    query = filter_query_for_user(query, q[9])             
    
    # record the time taken to respond
    start_time = time.time()
    predicted = ask_label(q[0], query) # ask the model to predict the label
    end_time = time.time()
...
mat = confusion_matrix(labels, y_true, y_pred)
