...
# these are used to build the questions (only devices than can be chosen are from lighting)
people = ["Dave", "Kate", "George", "Henry", "Evie"]
devices = ["living room light", "kitchen light", "hallway light", "garden light",
           "playroom light", "master bedroom light", "George's bedroom light", 
              "Henry's bedroom light", "Evie's bedroom light"]
        ...
        "messages": [
            {
                ...
                # contains an empty lighting policy set
...
# return the breakdown of each label
breakdown = " | ".join(f"{sum(1 for t in y_pred if t == label)} {label}" for label in labels)
print(f"answer breakdown: {breakdown}")
print()
